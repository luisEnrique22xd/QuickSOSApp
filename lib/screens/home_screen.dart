  ...
  Widget _buildAlertStats() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        Map<String, int> counts = {'Incendio': 0, 'Robo': 0, 'Accidente': 0};
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          UsernameLookup(
                            userId: selectedAlert!['createdBy'] ?? '',
                          ),
                        ],
                      ),
                    ],
              ...
              return SingleChildScrollView(
                child: Column(
                  children: [
                    ...
                    const SizedBox(
                      height: 200,
                    ), // espacio extra para panel flotante
                  ],
                ),
              );
            },
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            UsernameLookup(userId: selectedAlert!['uid'] ?? ''),
                          ],
                        ),
                      ],
                    ),
...
class UsernameLookup extends StatelessWidget {
  final String userId;

  const UsernameLookup({super.key, required this.userId});

  // Función para buscar el nombre de usuario en la colección 'users'
  Future<String> fetchUsername(String uid) async {
    ...
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        // Asumo que el campo donde guardaste el nombre es 'username'
        return data?['username'] ?? 'Usuario ID: $uid';
      }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<String>(
      ...
      builder: (context, snapshot) {
        ...
        // Muestra el nombre o el error
        return Text(
          snapshot.data ?? 'Usuario Desconocido',
          style: const TextStyle(
            color: Colors.white,
            fontWeight: FontWeight.bold,
          ),
